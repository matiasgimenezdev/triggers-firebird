...
BEGIN
    ...
    SELECT 
        COALESCE(SUM(IIF(EXTRACT(MONTH FROM F.FECHA) = 1, F.IMPORTE, 0)), 0), 
        COALESCE(SUM(IIF(EXTRACT(MONTH FROM F.FECHA) = 2, F.IMPORTE, 0)), 0),
        COALESCE(SUM(IIF(EXTRACT(MONTH FROM F.FECHA) = 3, F.IMPORTE, 0)), 0),
        COALESCE(SUM(IIF(EXTRACT(MONTH FROM F.FECHA) = 4, F.IMPORTE, 0)), 0),
        COALESCE(SUM(IIF(EXTRACT(MONTH FROM F.FECHA) = 5, F.IMPORTE, 0)), 0),
        COALESCE(SUM(IIF(EXTRACT(MONTH FROM F.FECHA) = 6, F.IMPORTE, 0)), 0),
        COALESCE(SUM(IIF(EXTRACT(MONTH FROM F.FECHA) = 7, F.IMPORTE, 0)), 0),
        COALESCE(SUM(IIF(EXTRACT(MONTH FROM F.FECHA) = 8, F.IMPORTE, 0)), 0),
        COALESCE(SUM(IIF(EXTRACT(MONTH FROM F.FECHA) = 9, F.IMPORTE, 0)), 0),
        COALESCE(SUM(IIF(EXTRACT(MONTH FROM F.FECHA) = 10, F.IMPORTE, 0)), 0),
        COALESCE(SUM(IIF(EXTRACT(MONTH FROM F.FECHA) = 11, F.IMPORTE, 0)), 0),
        COALESCE(SUM(IIF(EXTRACT(MONTH FROM F.FECHA) = 12, F.IMPORTE, 0)), 0)
        FROM TBL_FACTURA F WHERE (EXTRACT(YEAR FROM F.FECHA) = :ANIO) 
        INTO :MES01, :MES02, :MES03, :MES04, :MES05, :MES06, :MES07, :MES08,
